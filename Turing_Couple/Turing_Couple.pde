// Turing patterns centered couple //

int scl = 5, dirs = 9, lim = 128;
int dx, dy, w, h, s;
float[] pat;
float[] pnew;
float[][] pmedian;
...
void setup() {
  ...
  pat = new float[s];
  float val = random(TWO_PI);
  pat[((w/2)+(w/7))+(h/2)*w] = val;
  pat[((w/2)-(w/7))+(h/2)*w] = val;
...
void draw() {
  ...
  for(int l=0; l<s; l++) { 
   ...
   for(int i=imin; i<=imax; i++) pnew[l] += prange[l][i] * pvar[l][i] / 3.0;
