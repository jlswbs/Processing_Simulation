// Lenia organic cellular automata (multi-ring kernel) //
...
float kernelProfile(float d) {
  
  float k1 = gauss(d, 0.30, 0.04);
  float k2 = 0.7 * gauss(d, 0.60, 0.07);
  float k3 = 0.4 * gauss(d, 0.90, 0.05);
  return max(0, k1 + k2 + k3);
